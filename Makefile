...
#default C++ version
c: main.o rk_c.o input.o dataset.o fiber.o rk_kernel_c.o
	g++ main.o input.o rk_c.o rk_kernel_c.o dataset.o fiber.o -o rk

#general objects
	
main.o: main.cpp input.h output.h rk.h dataset.h
	g++ main.cpp -Wall -ansi -pedantic -c
	
input.o: input.cpp input.h dataset.h
	g++ -c -Wall -ansi -pedantic input.cpp
	
output.o: output.cpp output.h dataset.h
	g++ -c -Wall -ansi -pedantic output.cpp

dataset.o: dataset.cpp dataset.h
	g++ -c -Wall -pedantic dataset.cpp

fiber.o: fiber.cpp fiber.h
	g++ -c -Wall -pedantic fiber.cpp
...
cuda: main.o input.o dataset.o rk_cuda.o rk_cuda_kernel.o fiber.o output.o
	nvcc -g main.o input.o rk_cuda_kernel.o rk_cuda.o dataset.o fiber.o output.o -o rk ${COMPUTE_CAPABILITY}
	
rk_cuda_kernel.o: cuda/rk_kernel.cu cuda/rk_kernel.h dataset.h
	nvcc -c -g cuda/rk_kernel.cu -o rk_cuda_kernel.o ${COMPUTE_CAPABILITY}

rk_cuda.o: cuda/rk.cpp cuda/rk_kernel.h rk.h dataset.h
